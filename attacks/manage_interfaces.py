...
class interfaceObj(object):
    ...
    def showLogo(self):
        ...
        startupInfo = returnGreenDeep(self.colors,"Developed by ANJEESHNU, Using airmon-ng, iproute2 and net-tools. 2021-2021.")
    ...
    def launchAirodumpSession(self):
        if self.cardSelected():
            try:
                bashRun("~/.godseye/air/*/src/airodump-ng {}".format(
                    self.currentCard
                ))
    ...
    def launchWashSession(self):
        if self.cardSelected():
            try:
                createNewLine()
                bashRun("~/.godseye/wps/*/src/wash -i {} -F".format(
                    self.currentCard
                ))
            except(KeyboardInterrupt, EOFError):
                pass
    ...
    def launchPacketTest(self):
        if self.cardSelected():
            try:
                bashRun("~/.godseye/air/*/src/aireplay-ng -9 {}".format(
                    self.currentCard
                ))
            except(KeyboardInterrupt, EOFError):
                pass
