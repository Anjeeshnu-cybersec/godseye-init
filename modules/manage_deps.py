...
def quickCheckDepsInstalled():
    gitBinaryPaths = " ~/.godseye/air/*/src/aircrack-ng"
    gitBinaryPaths += " ~/.godseye/wps/*/src/reaver"
    gitBinaryPaths += " ~/.godseye/magic/*/pixiewps"
    gitBinaryPaths += " ~/.godseye/mdk/*/src/mdk4"
...
def cloneAircrackDeps():
    ...
    aircrackCloneAndBuild = "mkdir -p ~/.godseye/air && cd ~/.godseye/air && "
...
def cloneReaverDeps():
    ...
    reaverCloneAndBuild = "mkdir -p ~/.godseye/wps && cd ~/.godseye/wps && "
...
def clonePixiewpsDeps():
    bashRun("rm ~/.godseye/magic/ -r 2>/dev/null")
    pixiewpsCloneAndBuild = "mkdir -p ~/.godseye/magic && cd ~/.godseye/magic && "
...
def cloneMDK4Deps():
    ...
    bashRun("rm ~/.godseye/mdk/ -r 2>/dev/null")
    mdkCloneAndBuild = "mkdir -p ~/.godseye/mdk && cd ~/.godseye/mdk && "
...
def gitDeps():
    directoryMap = \
    {"aircrackDir" : "~/.godseye/air/*/src/aircrack-ng",
    "reaverDir" : "~/.godseye/wps/*/src/reaver",
    "pixiewpsDir" : "~/.godseye/magic/*/pixiewps",
    "mdk4Dir" : "~/.godseye/mdk/*/src/mdk4"}
    cloneFunctionMap = \
