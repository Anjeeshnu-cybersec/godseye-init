...
from modules.term_colors import printInfo
import os
import subprocess
import time
import sys
...
def displayWarranty():
    print("    %sGODSEYE Copyright (C) 2021 ANJEESHNU" %(col.red,col.endl))
    print("    %sThis program comes with ABSOLUTELY NO WARRANTY;" %(col.red,col.endl))
...
class functions:
    ...
    def displayCurrentPlace(prompt,*args):
        ...
        ")" + BASH_PROMPT
        return currentPlace
    #A basic check to see if godseye already exists in /usr/local/bin
    def checkExistingSymlink():
        if bashReturnValue("ls /usr/local/bin/godseye") == "0":
            return True
        else:
            return False
    ...
    def fetchGitHubDeps():
        ...
        try:
            modules.createNewLine()
            modules.printBlue(col, "This will clone godseye, reaver, pixiewps and mdk4 from GitHub.")
    ...
    def removeResidualFiles():
        ...
        if modules.yesNo("Confirm delete all residual files?", col):
            bashRun("rm ~/.godseye/ -rf 2>/dev/null")
...
class main:
    ...
    def removeSymlink():
        ...
        modules.printBlue(col,"Unless you already have another godseye in /usr/local/bin.")
        modules.createNewLine()
        try:
            if modules.yesNo("Remove the symbolic link?",col):
                if functions.checkExistingSymlink():
                    modules.createNewLine()
                    modules.printSuccess(col, "Found godseye in /usr/local/bin.")
                    if bashReturnValue("rm -f /usr/local/bin/godseye") == "0":
                        modules.printSuccess(col, "Successfully deleted the symlink.")
                        modules.printGreen(col, "Typing 'godseye' will no longer invoke it.")
                    else:
                        raise ValueError
                else:
            ...
            modules.printYellow(col, "Try running this manually: unlink /usr/local/bin/godseye")
        finally:
            functions.menuOrExit()
    ...
    def createSymlink():
        clearTerm()
        modules.createNewLine()
        modules.printBlue(col,"Creating a symbolic link allows you to run godseye from anywhere.")
        modules.printBlue(col,"Essentially, this adds an entry in /usr/local/bin.")
        modules.printBlue(col,"Next time you want to run godseye, just type 'godseye'.")
        ...
        try:
            ...
            if modules.yesNo("Create the symbolic link?",col):
                if not functions.checkExistingSymlink():
                    modules.createNewLine()
                    modules.printSuccess(col, "Adding Symbolic link -> /usr/local/bin/godseye")
                    if bashReturnValue("ls /usr/local/bin") == "0":
                        if bashReturnValue("echo $PATH | grep \"/usr/local/bin\"") == "0":
                            if bashReturnValue("ln -sf $(find $(pwd) -name {}) /usr/local/bin/godseye".format(FILE_NAME)) == "0":
                                modules.printSuccess(col, "Successfully created the symlink.")
                                modules.printGreen(col, "Now you can type 'godseye' from anywhere.")
                                modules.printGreen(col, "Go ahead, quit and try typing: godseye")
                            else:
                                raise ValueError
                        else:
                            raise FileNotFoundError
            ...
            modules.printYellow(col, "Try running this: unlink /usr/local/bin/godseye")
        except(ValueError):
            modules.createNewLine()
            modules.printYellow(col, "An error occured while creating the symlink.")
            modules.printYellow(col, "Run this manually:")
            modules.printYellow(col, "ln -sf $(find $(pwd) -name {}) /usr/local/bin/godseye".format(
                FILE_NAME))
        except(FileNotFoundError):
            modules.createNewLine()
            ...
            modules.printYellow(col, "You're probably running this using 'godseye'.")
            modules.printYellow(col, "That won't work. You'll have to run it manually for this occasion.")
            modules.printYellow(col, "Please head to where godseye is stored.")
            modules.printYellow(col, "Then run it manually with: ./godseye")
...
def mainMenu():
    try:
        ...
        if os.getuid() != 0:
            ...
            print("\t{0}->{1} [{0}i{1}] If you have symlinked the program, just do '{2}sudo godseye{1}' \n".format(
                col.blue,
                col.endl,
                col.red))
        ...
        menuTextItemArray = \
    [[col.pink_deep,'to crack WPA/WPA2','1','main.aircrackng'],
    [col.blue_deep,'to crack WPS','2','main.reaver'],
    ...
    [col.blue_deep,'Download and build the dependencies from kali.','7','functions.fetchGitHubDeps'],
